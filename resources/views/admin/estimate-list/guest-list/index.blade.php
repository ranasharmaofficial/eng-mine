    ...
    <div class="content container-fluid">
          ...
          <div class="col">
            <div class="breadcrumb ">
              <a href="{{ url('admin/dashboard') }}">
                <i class="fa fa-home" aria-hidden="true"></i> Dashboard </a> / Annonymous List
            </div>
          </div>
          {{--<div class="col">
      ...
      <div class="main-panel card-header ">
        <div class="row gutters-5 align-items-center">
          <div class="col">
            <h5 class="mb-md-0 h6">Annonymous List</h5>
          </div>
          <div class="col text-right">
            <a href="{{ route('admin.guestExport') }}" class="btn btn-circle btn-info h-35" download>
        ...
        <div class="table-responsive">
          <table class="table table-striped fs-13 lsp-5">
            ...
            <tbody> 
		@foreach($guest_list as $key => $item) 
			@php
				$noofEstimate = \App\Models\TempOrder::where('user_id', $item->id)->count();
			@endphp
			<tr>
                ...
                <td>
					<a class="btn btn-primary btn-sm p-1 mx-1" href="{{ url('admin/customer/view-guest-estimate/'.$item->id) }}">View&nbsp;Estimate</a>
        ...
        <div class="aiz-pagination mt-3 w-100">
			<div class="row justify-content-between">
				<div class="col-sm-6">
					<div class="dataTables_info fs-13 fw-bold" id="geniustable_info" role="status" aria-live="polite">Showing {{ $guest_list->currentPage() }} to {{ $guest_list->perPage() }} of {{ $guest_list->total() }} entries</div>
				</div>

				<div class="col-sm-6">
					{{ $guest_list->appends(request()->input())->links() }}
				</div>
			</div>
		</div>
